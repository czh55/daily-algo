...
校验三类内容（全量运行时）：
1. **精讲完整性** — 所有已发布页都是完整「变量语义法」精讲（不是缺精讲的占位页）
2. **首页一致性** — `index.html` 的归档/归类链接有效，且「按题型归类」数量与 `data/history.json` 一致
3. **题型配色** — `TYPE_CLASS_MAP` 中每个题型 class 在 `docs/style.css` 有 `.tag-xxx` 定义

- 退出码为 0（打印「✓ 校验通过」）才允许进入下一步提交。
- 若失败：
  - 缺「变量语义三句法 / 模拟答题者思考 / 落码步骤」→ 该天是占位页，**回到步骤 1**，补齐 `VAR_SEMANTICS_DATA` 精讲并 `--slug ... --force` 重新生成；
  - 首页一致性失败 → 检查链接是否指向存在的归档页、归类数量是否与历史一致（如手动改过数据/页面需重新生成）；
  - 配色缺失 → 在 `docs/style.css` 补 `.tag-xxx` 定义。
  修复到校验通过为止。**校验不通过绝不提交。**
